...
public class ChainReactionMain {
        ...
        private static void startHumanVsAiMode(int depth, int heuristicType) {
            for (int d : new int[]{1, 4,6}) {
                for (int h = 0; h <= 5; h++) {
                    ...
                    fileManager.writeGameState(uiState, true); // Initial state for human's turn
                    ...
                    while (uiState.getStatus() == GameState.GameStatus.ONGOING) {
                        ...
                        if (uiState.getCurrentPlayer() == Player.RED) {
                            ...
                            if (!possibleMoves.isEmpty()) {
                                ...
                                // ui.displayBoard(uiState); // Display board after random move and explosions
                                fileManager.writeGameState(uiState, true);
                            }
                            // Move humanMove = ui.getHumanMove();
                            // if (humanMove != null) {
                            //     uiState.makeMove(humanMove);
                            //     fileManager.writeGameState(uiState, true);
                            // }
                            ...
                            if (info != null) {
                                ...
                                // ui.displayBoard(uiState); // Display board after AI's move and explosions
                            }
                        }
                    }
    ...
    private static void startAiVsAiMode(int depth, int heuristicType) {
        ...
        // GameExperiment experiment = new GameExperiment(depth, heuristicType);
